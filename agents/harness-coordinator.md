...
model: sonnet
color: purple
tools: ["Read", "Write", "Grep", "Glob", "Bash", "Agent", "TaskCreate", "TaskUpdate", "Skill"]
---

# ⚠️ 重要：如何使用本 Agent

**本 Agent 是流程定义，不是可执行调度器。**

要实现真正的多 Agent 协作，你需要**启动 harness-scheduler**：
...
**或直接运行**：
```bash
npx tsx src/harness-scheduler.ts "build a blog with comments"
```
...
## Step 2: 碰撞对抗（Team 真对抗模式）

当前 Step 1 的 5 个 Agent 是**并行独立运行**的（互相看不到输出，这是正确的）。
Step 2 的碰撞对抗需要升级为**真正的 Agent-to-Agent 对抗**，而非 coordinator 代理对比。

### 实现方式（文件共享模式）

```
1. 创建辩论目录结构：
   .phase0-debate-{iteration_id}/
     round1/     # 5 个 Agent 的独立洞察
     round2/     # 5 个 Agent 的互相质疑
     round3/     # 5 个 Agent 的回应
     synthesis/  # Planner 的整合收敛

2. Round 1（独立洞察）：
   - 5 个 Agent 并行运行，每个 Agent 独立输出
   - 写入 round1/{agent_name}.md

3. Round 2（互相质疑）：
   - 每个 Agent 读取 round1/ 下其他 4 个 Agent 的输出
   - 必须对每个其他视角提出 ≥1 个具体质疑
   - 写入 round2/{agent_name}-challenges.md

4. Round 3（回应质疑）：
   - 每个 Agent 读取针对自己的 round2/ 质疑
   - 必须回应：接受/反驳/修正自己的观点
   - 写入 round3/{agent_name}-responses.md

5. Synthesis（整合收敛）：
   - Planner 读取所有 round3/ 文件
   - 提取共识点和分歧点
   - 输出收敛需求到 synthesis/converged-requirement.md
```
