...
## 核心原则
1. **强制垂直切片**: 每个 Task = 一个从用户操作到结果的完整路径。
   - ✅ "用户输入任务→点击添加→任务出现在列表中→刷新后仍在" (垂直)
   - ❌ "创建 HTML 文件" (水平切片，没有用户操作)
   - ❌ "编写所有CSS样式" (水平切片)
   - 如果 Task 描述不含用户动作 → 合并到相邻 Task
...
## Phase 2.5: CONTEXT.md 术语表
在输出 JSON 之前，先输出项目术语表（写入 .tasks/CONTEXT.md）:

## 术语表 (CONTEXT.md)
| 术语 | 定义 | 别名(禁止使用) |
|------|------|---------------|
| Task | 用户定义的待办事项单元 | 待办、todo、item |
| [项目特定术语] | [精确定义] | [容易混淆的别名] |

术语表目的: 确保 Planner/Generator/Evaluator 用同一种语言沟通。
